...
def main():
    ...
    if args.dataset == 'mnist':
        ...
        in_shape = (3, 32, 32)
    if args.dataset == 'glo':
        latentz_transforms = [transforms.Pad(12, 0), transforms.ToTensor(), lambda x: x.repeat((3, 1, 1))]
        ...
        args.nClasses = 10
        in_shape = (3, 32, 32)
    if args.dataset == 'vqvae':
        assert args.densityEstimation, "Currently tiny_imagenet is only supported for density estimation"
        ...
        trainset = vqvae_code.tiny_imagenet('./data/vqvae/train_c.pt', train=True, transform=transform_train_tiny_imagenet)
        testset = vqvae_code.tiny_imagenet('./data/vqvae/train_c.pt', train=False, transform=transform_train_tiny_imagenet)
        args.nClasses = 10
        in_shape = (3, 32, 32)
    else:
